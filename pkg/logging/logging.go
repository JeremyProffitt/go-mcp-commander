...
const (
	...
	LevelInfo
	// LevelAccess logs command execution operations
	LevelAccess
...
// Access logs command execution operations
...
// CommandExec logs a command execution (command and exit code, NEVER output content)
func (l *Logger) CommandExec(command string, workDir string, exitCode int, duration time.Duration, err error) {
	if err != nil {
		l.Access("CMD_EXEC command=%q workdir=%q exit_code=%d duration=%s error=%q", command, workDir, exitCode, duration, err.Error())
	} else {
		l.Access("CMD_EXEC command=%q workdir=%q exit_code=%d duration=%s", command, workDir, exitCode, duration)
	}
}

// CommandBlocked logs a blocked command attempt
func (l *Logger) CommandBlocked(command string, reason string) {
	l.Warn("CMD_BLOCKED command=%q reason=%q", command, reason)
}
...
func (l *Logger) LogStartup(info StartupInfo) {
	...
	if info.AllowedCmds.Value != "" {
		l.Info("Allowed Commands: %s [%s]", info.AllowedCmds.Value, info.AllowedCmds.Source)
	} else {
		l.Info("Allowed Commands: <all> [%s]", info.AllowedCmds.Source)
	}
	if info.BlockedCmds.Value != "" {
		l.Info("Blocked Commands: %s [%s]", info.BlockedCmds.Value, info.BlockedCmds.Source)
	} else {
		l.Info("Blocked Commands: <none> [%s]", info.BlockedCmds.Source)
	}
	l.Info("Default Timeout: %s [%s]", info.DefaultTimeout.Value, info.DefaultTimeout.Source)
	l.Info("Shell: %s [%s]", info.Shell.Value, info.Shell.Source)
	l.Info("----------------------------------------")
	l.Info("ENVIRONMENT")
	l.Info("----------------------------------------")
...
// CommandExec logs command execution using the default logger
func CommandExec(command string, workDir string, exitCode int, duration time.Duration, err error) {
	if defaultLogger != nil {
		defaultLogger.CommandExec(command, workDir, exitCode, duration, err)
	}
}

// CommandBlocked logs blocked command using the default logger
func CommandBlocked(command string, reason string) {
	if defaultLogger != nil {
		defaultLogger.CommandBlocked(command, reason)
	}
}
